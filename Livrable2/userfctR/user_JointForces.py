...
def user_JointForces(mbs_data, tsim):
    ...
    # Avant : R2 de Bras_sup_AV_G (q8) et R2 de Bras_sup_AV_D (q13)
    C_bar_av = um['FrontSuspension']['C_bar']
    delta_av = mbs_data.q[23] - mbs_data.q[17]       # différence d'angle G/D
    mbs_data.Qq[23]  += -C_bar_av * delta_av           # couple sur joint q[8]
    mbs_data.Qq[17] +=  C_bar_av * delta_av           # couple sur joint q[13]


    # Arrière : R1 de Bras_sup_AR_G (q24) et R1 de Bras_sup_AR_D (q28)
    C_bar_ar = um['RearSuspension']['C_bar']
    delta_ar = mbs_data.q[27] - mbs_data.q[31]
    mbs_data.Qq[27] += -C_bar_ar * delta_ar
    mbs_data.Qq[31] +=  C_bar_ar * delta_ar
...
"""
    ...
    if tsim > 1:  # Attente de 1 [s] pour que la voiture soit parfaitement stable.
    ...
    if tsim > 3.0:  # Attente de 3 [s] pour que la voiture soit parfaitement stable.
